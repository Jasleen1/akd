...
pub struct TreeNode {
    ...
    // ozks: Needs to include its current value as well as pointers to its left and right child i.e. labels.
    // Possibly also store the hashes of children. 
}
...
impl TreeNode {
    ...
    pub(crate) fn as_history_tree_node(&self) -> HistoryTreeNode {
        HistoryTreeNode { 
            label: self.label.clone(), 
            last_epoch: self.last_epoch.clone(), 
            birth_epoch: self.birth_epoch.clone(), 
            parent: self.parent.clone(), 
            node_type: self.node_type.clone() 
        }
    }

    pub(crate) fn from_history_tree_node(node: HistoryTreeNode) -> Self {
        Self { 
            label: node.label.clone(), 
            last_epoch: node.last_epoch.clone(), 
            birth_epoch: node.birth_epoch.clone(), 
    ...
    pub(crate) async fn write_to_storage<S: Storage + Send + Sync>(
        ...
        storage.set(DbRecord::HistoryTreeNode(self.as_history_tree_node())).await
    }
    ...
    pub(crate) async fn insert_single_leaf_helper<S: Storage + Sync + Send, H: Hasher>(
        ...
        if self.is_root() {
            ...
            *num_nodes += 1;
            ...
            // Ozks: Change to get_child (no need for epoch)
            let child_state = self
                .get_child_at_epoch::<_, H>(storage, self.get_latest_epoch(), dir_leaf)
                .await?;
            if child_state == None {
                new_leaf.parent = self.label;
                // Ozks: Can just change to set_child and include a pointer to children at any point
                ...
                if hashing {
                    ...
                    // Ozks: No longer needs epoch either
                    let mut new_self: TreeNode =
                        TreeNode::get_from_storage(storage, &NodeKey(self.label), epoch)
                            .await?;
                    new_self.update_hash::<_, H>(storage, epoch).await?;
                    *self = new_self;
                } else {
                    *self = TreeNode::get_from_storage(storage, &NodeKey(self.label), epoch)
                        .await?;
                }
        ...
        match dir_self {
            Some(_) => {
                ...
                let mut parent =
                    TreeNode::get_from_storage(storage, &NodeKey(self.parent), epoch)
                        .await?;
                debug!("BEGIN get direction at epoch {}", epoch);
                // ozks: change to get direction
                let self_dir_in_parent = parent.get_direction_at_ep(storage, self, epoch).await?;

                debug!("BEGIN create new node");
                let mut new_node =
                    TreeNode::new(lcs_label, parent.label, NodeType::Interior, epoch);
                ...
                *num_nodes += 1;
                ...
                if hashing {
                    ...
                    new_node =
                        TreeNode::get_from_storage(storage, &NodeKey(new_node.label), epoch)
                            .await?;
            ...
            None => {
                ...
                let mut child_node =
                    TreeNode::get_from_storage(storage, &NodeKey(child_st.label), epoch)
                        .await?;
    ...
    async fn update_hash_at_parent<S: Storage + Sync + Send, H: Hasher>(
        ...
        let parent =
            &mut TreeNode::get_from_storage(storage, &NodeKey(self.parent), epoch).await?;
    ...
    pub(crate) async fn set_child<S: Storage + Sync + Send, H: Hasher>(
        ...
        // That is, make sure after every call to TreeNode::new, there is a call to
        // set_state_map.
    ...
    /// This function is just a wrapper: given a [`TreeNode`], sets this node's latest value using
    ...
    pub(crate) async fn get_child_at_epoch<S: Storage + Sync + Send, H: Hasher>(
        ...
        match direction {
            Direction::None => Err(AkdError::TreeNode(
...
pub async fn get_empty_root<H: Hasher, S: Storage + Send + Sync>(
    ...
    let mut node = TreeNode::new(NodeLabel::root(), NodeLabel::root(), NodeType::Root, 0u64);
...
pub(crate) fn get_state_map_key(node: &TreeNode, key: u64) -> NodeStateKey {
